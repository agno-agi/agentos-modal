...
| `NEON_PROJECT_ID` | no | — | Set by `scripts/modal/up.sh` after provisioning the Neon project; `down.sh` reads it to delete the project. Absent when you created the database yourself. |
| `NEON_ORG_ID` | no | — | Neon organization for unattended creates — `neonctl projects create` prompts for an org otherwise (find yours with `neonctl orgs list`). |
